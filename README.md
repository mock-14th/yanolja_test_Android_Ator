...
#### *개발 도중 발생한 이슈와 해결 방법*
1. 스크롤을 하게 되면 appbar가 투명한 상ㅌ에서 흰색으로 변하는 효과를 주기 위해 많은 시간을 투자했다. coordination부터 collapsing까지 찬찬히 공부를 해보았는데, 실제 앱에서 구현하는 방법과는 움직임이 달랐다. 따라서, 결국은 스크롤을 리스너로 감지해 스크롤이 시작되면 appbar에 애니메이션을 주는 방향으로 코딩을 했다. 이것도 꽤 많은 시행착오가 있었는데, 다행히 원하는 기능을 구현하는데 성공했다. (setOnScrollChangeListener를 이용했다.)
